...
def test_sft_quality_card_matches_persisted_result() -> None:
    persisted = json.loads(
        (ROOT / "ai/results/sft-comparison-2026-09-04.json").read_text(encoding="utf-8")
    )
    gate = _gates()["sft"]
    metrics = gate["metrics"]

    assert gate["status"] == persisted["decision"] + "ed"
    assert metrics["base_reference_token_f1"] == persisted["base"]["reference_token_f1"]
    assert metrics["candidate_reference_token_f1"] == persisted["candidate"]["reference_token_f1"]
    assert metrics["delta_reference_token_f1"] == persisted["delta_reference_token_f1"]
    assert gate["run_id"] == persisted["run_id"]


def test_dpo_quality_card_matches_persisted_result() -> None:
    persisted = json.loads(
        (ROOT / "ai/results/dpo-comparison-2026-09-04.json").read_text(encoding="utf-8")
    )
    gate = _gates()["dpo"]
    metrics = gate["metrics"]
    ...
    assert metrics["sft_reference_token_f1"] == persisted["base"]["reference_token_f1"]
    assert metrics["dpo_reference_token_f1"] == persisted["candidate"]["reference_token_f1"]
    assert metrics["delta_reference_token_f1"] == persisted["delta_reference_token_f1"]
    assert gate["run_id"] == persisted["run_id"]
