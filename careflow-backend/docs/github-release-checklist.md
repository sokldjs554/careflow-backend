...
이 문서는 저장소를 공개하기 직전에 실행합니다. 현재 대화에서는 GitHub에 push하지 않았습니다.

## 1. 실기능 검증

- [ ] `.env`에 개인 `ANTHROPIC_API_KEY`를 로컬에서만 설정한다.
- [ ] 합성 대화로 `uv run python scripts/live_claude_check.py`를 실행한다.
- [ ] Claude 결과에 S/O/P와 세 section의 근거 sequence가 있는지 확인한다.
- [ ] 대화에 없던 진단·검사·약물·치료가 추가되지 않았는지 수동 검토한다.
...
- [ ] 실제 결과에 맞춰 README의 `미검증` 문구만 수정한다. 실패 결과도 삭제하지 않는다.
...
- [ ] AWS는 Terraform 시작점이고 실제 배포가 아님을 명시한다.
- [ ] Claude 외부 처리와 실제 의료정보 사용 금지 경계를 명시한다.
